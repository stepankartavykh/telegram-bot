...
from states.dialog_state_low import UserDialogLow
...
from keyboards.reply import low_keyboards
import datetime
import locale
...
def low_get_required_parameters(property_parameters: dict) -> tuple:
...
def low_command_start(message: Message) -> None:
    """ Обработка команды /low """
    Operations.add_entry(db, Entry("/low"))
    bot.send_message(message.chat.id, "Данная команда предоставит информацию о самых дешевых отелях.")
    bot.send_message(message.chat.id, "Введите город для поиска:")
    bot.set_state(message.from_user.id, UserDialogLow.city, message.chat.id)


@bot.message_handler(state=UserDialogLow.city)
@logger.catch
def low_get_city_and_put_calendar_for_date_in(message: Message) -> None:
    ...
    for location in city_locations:
        if location["regionNames"]["shortName"] == city:
            ...
            calendar_date_in, step_in = DetailedTelegramCalendar(min_date=datetime.date.today(), calendar_id=0,
                                                                 locale='ru').build()
...
@logger.catch
def low_get_date_in_and_put_calendar_for_date_out(call_in: CallbackQuery) -> None:
    """ Функция для обработки даты заезда. Идет запись даты в data. Вывод календаря для получения даты выезда. """
    result, key, step = DetailedTelegramCalendar(min_date=datetime.date.today(), calendar_id=0, locale='ru').process(
        call_in.data)
    if not result and key:
        ...
        with bot.retrieve_data(user_id=call_in.from_user.id, chat_id=call_in.message.chat.id) as data:
            ...
            data["month_in"] = month
            data["year_in"] = year
        calendar_date_out, step_out = DetailedTelegramCalendar(min_date=datetime.date.today(), calendar_id=1,
                                                               locale='ru').build()
...
@logger.catch
def low_get_date_out_and_ask_for_count_variants(call_out: CallbackQuery) -> None:
    ...
    result, key, step = DetailedTelegramCalendar(min_date=date_out_low_limit, calendar_id=1, locale='ru').process(
        call_out.data)
    if not result and key:
        bot.edit_message_text(
            f"Выберите {PERIODS[step]}",
        ...
        year, month, day = str(result).split('-')
        date_check_out = datetime.date(int(year), int(month), int(day))
        result = date_check_out.strftime("%d %B %Y (%A)")
        bot.edit_message_text(
        ...
        bot.set_state(call_out.from_user.id, UserDialogLow.count_hotels, call_out.message.chat.id)


@bot.message_handler(state=UserDialogLow.count_hotels)
def low_get_count_hotels_and_ask_for_limit_output_entries(message: Message) -> None:
    ...
    try:
        ...
        bot.set_state(message.from_user.id, UserDialogLow.count_photos, message.chat.id)
    except ValueError:
        bot.send_message(message.chat.id, "Неправильный ввод количества выводимых отелей. Введите заново:")


@bot.message_handler(state=UserDialogLow.count_photos)
def low_get_count_photos_and_get_confirmation(message: Message) -> None:
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Подтвердите выполнение запроса.", reply_markup=low_keyboards.confirmation)
    except ValueError:
        bot.send_message(message.chat.id, "Неправильный ввод количества фотографий одного отеля! Введите заново:")


@bot.callback_query_handler(func=lambda call: call.data == "quit_low_command")
def low_quit_command(call: CallbackQuery) -> None:
    """ Обработка выхода из команды /low. Удаляется состояние пользователя. """
    bot.send_message(call.message.chat.id, "Осуществлен выход из команды low.")
    bot.delete_state(call.from_user.id)


@bot.callback_query_handler(func=lambda call: call.data == "get_hotels_list")
@logger.catch
def low_get_properties_from_api(call: CallbackQuery) -> None:
    ...
    for item_property in properties:
        try:
            property_id, property_name, data_price = low_get_required_parameters(item_property)
    ...
    if len(names_prices) == 0:
        ...
        low_quit_command(call)
        return None
    names_prices = sorted(names_prices, key=lambda elem: elem[2])[:limit]
    with bot.retrieve_data(user_id=call.from_user.id, chat_id=call.message.chat.id) as data:
        data["list_shown_hotels"] = names_prices
        data["hotels_list_keyboard"] = low_keyboards.get_keyboard_with_properties(names_prices)
    bot.send_message(
        call.message.chat.id, "Список отелей:",
        reply_markup=low_keyboards.get_keyboard_with_properties(names_prices)
    )
    bot.send_message(
        call.message.chat.id,
        "Если хотите увидеть фотографии отеля, нажмите на кнопку с названием.",
        reply_markup=low_keyboards.abort_low_command_keyboard
    )


@bot.callback_query_handler(func=lambda call: "shown_hotel_buttonLow" in call.data)
@logger.catch
def low_get_property_details_from_api(call: CallbackQuery) -> None:
    ...
    if not this_hotel_already_shown:
        ...
        bot.send_message(call.message.chat.id, "Будете дальше смотреть отели?",
                         reply_markup=low_keyboards.quit_or_continue)
    else:
        bot.send_message(call.message.chat.id, "Вы уже просмотрели фотографии данного отеля.")


@bot.callback_query_handler(func=lambda call: call.data == "continue_search")
def low_continue_search_hotel(call: CallbackQuery) -> None:
...
@bot.callback_query_handler(func=lambda call: call.data == "break_search")
def low_break_search(call: CallbackQuery) -> None:
